...
const Card = ({ _id, details, name, image, ProfileImage }) => {
        ...
        <div className='h-auto mt-5 w-full card rounded-[10px]   text-[#e1d9d1]'>
            <div className='space-y-5 pb-5 p-5 bg-transparent'>
                <div className='flex justify-between'>
                    <div className='flex gap-3 bg-transparent'><img src={ProfileImage} className='w-[40px] h-[40px] rounded-full bg-transparent' /> <h1 className='mt-2'>{name}</h1> <VscVerifiedFilled className='sm:mt-2.5 mt-3' /></div>
                    <BsThreeDots className='text-[20px]'/>
                </div>
                <p className=''>{details}</p>
            </div>
            <img src={image} className='rounded h-[270px] bg-transparent w-full contain' onClick={() => setImageModal(true)} />
            ...
            <div className='flex justify-between w-full p-5 mt-2 text-[20px]'>
                <label onClick={like} className={`flex gap-2 ${isActive && 'text-[#43affc]'}`}> <AiFillLike className={`bg-transparent `} /> <span className='text-[15px]'>{likes} Likes</span></label>
